...
public interface ReserveListDao extends JpaRepository<ReserveList, Long>{
	...
	@Query(value="SELECT * FROM reserve_list WHERE store_id =:store_id AND reserve_date=:reserve_time AND nickname=:nickname", nativeQuery =true)
	...
	@Query(value="SELECT * FROM reserve_list WHERE nickname =:nickname AND reserve_date >=now()", nativeQuery = true)
